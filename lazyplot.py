...
DF_PLZ = pd.read_csv(PLZGEODATEN, sep=';', dtype={0: 'str', 1: 'str', 2: 'float', 3: 'float'}, encoding='latin-1').set_index('Plz')
...
class LazyPlot:
    ...
    def date_plot(self, 
        ...
        if date_column:
            if isinstance(date_column, str):
                df_plot.loc[:, date_column] = pd.to_datetime(df_plot[date_column], dayfirst=dayfirst)
        ...
        for i, value in enumerate(values):
            ax = df_plot[value].resample('D').sum().plot(color=colors[i],
                                                         label=value)
        if show_legend:
            ax.legend()
    ...
    def group_by_plz(self):
        df_plz = DF_PLZ.copy()
        s_umsatz = self._df.fillna(value={'plz':'XXXXX'}).groupby('plz')['betrag'].sum().fillna(0.0)
        df_plz['betrag'] = s_umsatz
        return df_plz[~df_plz.betrag.isnull()]
    
    def plot_to_germany(self,
                        figsize=(15, 6) 
        ):
        df_plot = self._df.copy()
        ...
        gdf_plot = gpd.GeoDataFrame(df_plot, geometry=[Point(x,y) for x,y in zip(df_plot.lon, df_plot.lat)])
